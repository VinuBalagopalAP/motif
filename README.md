...
- **Automated API Key Rotation**: The Gemini provider implements an intelligent `try/catch` round-robin loop. If the free-tier rate limit is exceeded (`429 Too Many Requests`), it instantly cycles through backup keys and seamlessly restarts the generation, effectively multiplying free-tier quotas and guaranteeing zero dropped payloads.
...
- **Zero-Cost WebSocket Hydration**: The UI completely bypasses aggressive HTTP polling by utilizing **Supabase Realtime (WebSockets)** for efficient, push-based updates. When a background generation job finishes, the browser instantly receives a payload, drastically preserving Vercel Serverless compute quotas.
...
- **Google Gemini 2.5 Flash**: Used as the primary video orchestration engine (and chat fallback). The Free Tier is strictly limited to 15 requests per minute. However, as noted in the Key Features, Motif actively bypasses this bottleneck using its **Automated Key Rotation System** to cycle through your `.env.local` keys.
