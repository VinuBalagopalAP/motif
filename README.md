...
- **Strict Architecture**: The project has undergone rigorous enterprise-grade decomposition. The main interface (`page.tsx`) acts purely as a presentation layer, completely decoupling business logic and API orchestration into atomic `Zustand` stores (`useAppStore`, `useChatStore`) and a dedicated `useChatActions` hook. Major UI boundaries (`ChatFeed`, `VideoEditor`, `ChatInput`, `Sidebar`, and `Modals`) are fully isolated and typed to ensure extreme stability.
...
- **Zero-Cost WebSocket Hydration**: The UI completely bypasses aggressive HTTP polling by utilizing **Supabase Realtime (WebSockets)** for efficient, push-based updates. When a background generation job finishes, the browser instantly receives a payload, drastically preserving Vercel Serverless compute quotas and database reads.
